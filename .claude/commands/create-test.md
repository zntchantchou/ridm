## First step: observe

First take a look at the changes that may have taken place in the application that seem relevant to this testing task

## Follow the /tasks/test-template.md

## About the arguments

The second argument is the functionality that is being tested: $2, the topic of this task
The optional third argument is the guidelines file to help with specific informations for this test creation: $2

## Create a checklist not exceeding 10 test scenarios

Update this checklist once all the tests have been validated.

## Uncertainties

- Mark any uncertainty in the check list with [UNCERTAINTY] that must be resolved by updating the test task with clarifications.
