...
class Pulses {
  ...
  update(stepper: Stepper, oldSteps: number, newSteps: number): void {
    ...
    try {
      ...
      console.error(`[Pulses] Error during update from ${oldSteps} to ${newSteps}:`, error);
  ...
  private promoteChild(parent: Pulse): void {
    ...
    const siblings = parent.getSubs()!.filter(sub => sub.steps !== largestChild.steps);
  ...
  validateState(): boolean {
    ...
    for (const steps of this.leadSteps) {
      const pulse = this.pulses.get(steps);
      if (!pulse) {
        console.error(`[Validation] Lead step ${steps} not found in pulses Map`);
        return false;
      }
      if (!pulse.lead) {
        console.error(`[Validation] Pulse ${steps} in leadSteps but lead=false`);
        return false;
      }
    }
    ...
    for (let i = 0; i < this.leadSteps.length - 1; i++) {
      if (this.leadSteps[i] <= this.leadSteps[i + 1]) {
        console.error(`[Validation] leadSteps not sorted descending at index ${i}`);
        return false;
      }
    }
    ...
    for (const [steps, pulse] of this.pulses.entries()) {
      if (pulse.lead && !this.leadSteps.includes(steps)) {
        console.error(`[Validation] Lead pulse ${steps} not in leadSteps array`);
        return false;
      }
    }
